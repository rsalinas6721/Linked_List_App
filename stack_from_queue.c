...
void listQueueAddBack (struct Queue* queue, TYPE value) 
{
    ...
    assert(newLink != 0); /*allocate memory for new link*/
    newLink->value = value;
    newLink->next = NULL; /*set equal to NULL */
    queue->tail->next = newLink; /*make tail ptr point to new link*/
    queue->tail = newLink;
}
...
TYPE listQueueFront(struct Queue* queue) 
{
    ...
    int number = queue->head->next->value; /*get element in front of queue*/
    return number;
}
...
TYPE listQueueRemoveFront(struct Queue* queue) 
{
    ...
    int returnVal = queue->head->next->value;
...
{
	struct Queue *tempStack = stack->q1;
	stack->q1 = stack->q2;
	stack->q2 = tempStack;
}
...
{
    TYPE number = listQueueFront(stack->q1);
    return number;
}
